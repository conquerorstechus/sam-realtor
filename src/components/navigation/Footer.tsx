...
export function Footer() {
  return (
    <footer className="mt-20 border-t border-black/8 bg-surface-2">
      <Container className="grid gap-10 py-14 md:grid-cols-3">

        {/* Brand */}
        <div>
          <Link href="/" className="flex items-center gap-3">
            <Image src="/logo.png" alt="Tampa Realty Pros" width={44} height={44} className="h-11 w-11 object-contain" />
        ...
        <div>
          ...
          <Link
            href="/contact"
            className="mt-4 inline-flex rounded-xl bg-gradient-to-r from-[#C63A2B] to-[#F28C45] px-5 py-3 text-sm font-extrabold text-white shadow-[0_8px_20px_rgba(232,100,42,0.25)] transition hover:brightness-110"
          >
      ...
      <div className="border-t border-black/8 py-6">
        <Container className="flex flex-col gap-2 text-xs text-muted sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-4">
            <Link href="/properties" className="transition hover:text-accent">Properties</Link>
